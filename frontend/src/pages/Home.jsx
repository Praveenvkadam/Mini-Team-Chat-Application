...
export default function Home() {
    ...
    (channelId, opts = {}) => {
      ...
      if (opts.left && opts.user && selectedChannel && selectedChannel._id === id) {
        setMembers((prev) => {
          ...
          const found = prev.find((m) => String(m._id || m.id) === uid);
          if (found) {
            return prev.map((m) =>
              String(m._id || m.id) === uid ? { ...m, active: false, left: true } : m
            );
          } else {
            return [...prev, { ...opts.user, active: false, left: true }];
          }
        });
      }
  ...
  const renderMemberCard = (m) => {
    ...
    const online = userId && onlineUserIds.includes(String(userId));
    ...
    return (
      <div key={userId} className="flex items-center gap-3 p-2 bg-gray-50 rounded">
        ...
        <span className={`text-xs font-semibold ${statusColorClass}`}>{statusText}</span>
      </div>
    );
  };
